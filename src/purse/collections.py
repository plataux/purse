...
class RedisHash(Generic[T]):
    __slots__ = ('redis', 'hkey', '_value_type')

    def __init__(self, redis: Redis, hkey, value_type: Type[T]):
        self.redis: Redis = redis
        self.hkey = hkey
        self._value_type: Type[T] = value_type

    async def set(self, key, value: T):
        return self.redis.hset(self.hkey, key=key, value=_obj_to_raw(self._value_type, value))

    async def update(self, mapping: Mapping[Any, T]):
        return await self.redis.hset(self.hkey, mapping=_dict_to_raw(self._value_type, mapping))

    async def get(self, key) -> Union[T, Any]:
        raw_item = await self.redis.hget(self.hkey, key=key)
    ...
    async def delete(self, key):
        ...
        :return:
        """
        return await self.redis.hdel(self.hkey, key)

    async def clear(self):
        return await self.redis.delete(self.hkey)

    async def contains(self, key) -> bool:
        return bool(await self.redis.hexists(self.hkey, key))

    async def len(self) -> int:
        d: int = await self.redis.hlen(self.hkey)
        return d
    ...
    async def dict(self) -> Dict[str, T]:
        ...
        raw: Dict[bytes, Union[str, bytes]] = await self.redis.hgetall(self.hkey)
...
:return: AsyncIterator to be used with ``async for`` constructs
        ...
        raw_it = self.redis.hscan_iter(
            self.hkey, match=match, count=batch_hint)
...
class RedisSet(Generic[T]):
    __slots__ = ('redis', 'skey', '_value_type')

    def __init__(self, redis: Redis, skey: str, value_type: Type[T]):
        self.redis: Redis = redis
        self.skey: str = skey
        self._value_type: Type[T] = value_type
    ...
    async def update(self, *members: T):
        return await self.redis.sadd(self.skey, *_list_to_raw(self._value_type, members))

    async def clear(self):
        return await self.redis.delete(self.skey)

    async def remove(self, *members: T):
        return await self.redis.srem(self.skey, *_list_to_raw(self._value_type, members))

    async def contains(self, member: T) -> bool:
        c: bool = await self.redis.sismember(self.skey, _obj_to_raw(self._value_type, member))
        return c

    async def len(self):
        return await self.redis.scard(self.skey)
    ...
    def values(self, match: Union[str, None] = None, batch_hint=None) -> AsyncIterator[T]:
        ...
        raw_it = self.redis.sscan_iter(self.skey, **kwargs)
...
class RedisSortedSet(Generic[T]):
    __slots__ = ('redis', 'zkey', '_value_type')

    def __init__(self, redis: Redis, zkey: str, value_type: Type[T]):
        self.redis: Redis = redis
        self.zkey: str = zkey
        self._value_type: Type[T] = value_type
    ...
    async def add(self, members: Dict[T, float], nx=False, xx=False, ch=False):
        ...
        return await self.redis.zadd(self.zkey, raw_members, nx=nx, xx=xx, ch=ch)
    ...
    async def increment(self, members: Dict[T, float]) -> Dict[T, float]:
        ...
        if cx == 1:
            ...
            new_score = await self.redis.zincrby(self.zkey, v, _obj_to_raw(self._value_type, k))
            return {k: new_score}
            ...
            async with self.redis.pipeline(transaction=True) as pipe:
                for k, v in raw_members.items():
                    pipe = pipe.zincrby(self.zkey, v, k)
    ...
    async def remove(self, *members: T):
        return await self.redis.zrem(self.zkey, *_list_to_raw(self._value_type, members))
    ...
    async def score(self, members: List[T]) -> Dict[T, float]:
        ...
        if len(members) == 1:
            score = await self.redis.zscore(self.zkey, _obj_to_raw(self._value_type, members[0]))
    ...
    async def rank(self, member: T, descending=False) -> int:
        if not descending:
            r: int = await self.redis.zrank(self.zkey, _obj_to_raw(self._value_type, member))
        else:
            r = await self.redis.zrevrank(self.zkey, _obj_to_raw(self._value_type, member))
        return r
    ...
    async def slice_by_rank(self, min_rank: int, max_rank: int,
        ...
        raw_result = await self.redis.zrange(
            self.zkey, start=min_rank, end=max_rank,
    ...
    async def slice_by_score(self, min_score: float,
        ...
        if not descending:
            raw_result = await self.redis.zrangebyscore(self.zkey, min=min_score, max=max_score,
                                                        start=offset, num=count,
                                                        withscores=True)
        else:
            raw_result = await self.redis.zrevrangebyscore(self.zkey, min=max_score,
                                                           max=min_score,
                                                           start=offset, num=count,
                                                           withscores=True)
    ...
    async def clear(self):
        return await self.redis.delete(self.zkey)

    async def len(self):
        return await self.redis.zcard(self.zkey)

    async def pop_max(self, count=1) -> Dict[T, float]:
        raw_result: List[Tuple[Any, Any]] = await self.redis.zpopmax(self.zkey, count=count)
        result: Dict[T, float] = {}
    ...
    async def pop_min(self, count=1) -> Dict[T, float]:
        raw_result: List[Tuple[Any, Any]] = await self.redis.zpopmin(self.zkey, count=count)
        result: Dict[T, float] = {}
        for k, v in raw_result:
            result[_obj_from_raw(self._value_type, k)] = v
        return result
    ...
    async def blocking_pop_min(self, timeout=0) -> Tuple[T, float]:
        val = await self.redis.bzpopmin(keys=[self.zkey], timeout=timeout)
        return _obj_from_raw(self._value_type, val[1]), val[2]

    async def blocking_pop_max(self, timeout=0) -> Tuple[T, float]:
        val = await self.redis.bzpopmax(keys=[self.zkey], timeout=timeout)
        return _obj_from_raw(self._value_type, val[1]), val[2]
    ...
    def values(self, match=None, batch_hint=None) -> AsyncIterator[Tuple[T, float]]:
        ...
        raw_it = self.redis.zscan_iter(self.zkey, **kwargs)
...
class RedisList(Generic[T]):
    """
    acts as a list, and as a deque
    """

    __slots__ = ('redis', 'lkey', '_value_type')

    def __init__(self, redis: Redis, lkey: str, value_type: Type[T]):
        self.redis: Redis = redis
        self.lkey: str = lkey
        self._value_type: Type[T] = value_type

    async def clear(self):
        await self.redis.delete(self.lkey)
    ...
    async def insert(self, index: int, item: T):
        ...
        if index == 0:
            await self.redis.lpush(self.lkey, raw_value)
            return

        pipe: Any
        async with self.redis.pipeline(transaction=False) as pipe:
            pipe.llen(self.lkey)
            pipe.lindex(self.lkey, index)
            list_len, pivot = await pipe.execute()

        if index >= list_len:
            await self.redis.rpush(self.lkey, raw_value)
            return
        ...
        async with self.redis.pipeline(transaction=True) as pipe:
            # pipe.multi()
            uid = str(uuid4())
            pipe.lset(self.lkey, index, uid)
            pipe.linsert(self.lkey, 'BEFORE', uid, raw_value)
            pipe.lset(self.lkey, index + 1, pivot)
            await pipe.execute()

    async def setitem(self, index: int, value: T):
        res = await self.redis.lset(self.lkey, index, _obj_to_raw(self._value_type, value))
        return res

    async def getitem(self, index: int) -> T:
        res = await self.redis.lindex(self.lkey, index)
    ...
    async def slice(self, start: int, stop: int) -> List[T]:
        raw_res = await self.redis.lrange(self.lkey, start, stop-1)
    ...
    def values(self, batch_size: Union[int, None] = 10) -> AsyncIterator[T]:
        ...
        async def _typed_iter():
            if not batch_size or (list_len := await self.len()) <= batch_size:
                items = await self.redis.lrange(self.lkey, 0, -1)
                for item in items:
                ...
                for _ in range(full_batches):
                    items = await self.redis.lrange(self.lkey, last_pos, last_pos + batch_size - 1)
                    for item in items:
                        yield _obj_from_raw(self._value_type, item)
                    last_pos += batch_size

                if remainder:
                    items = await self.redis.lrange(self.lkey, last_pos, -1)
                    for item in items:
                        yield _obj_from_raw(self._value_type, item)
